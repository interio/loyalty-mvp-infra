...
```
3) Apply backend migrations (required for a fresh DB):
```
cd ..
docker run --rm \
...
cd loyalty-mvp-infra
```
4) Local API with hot reload:
```
...
5) Stop services: `./dev.sh down`. Tail logs: `./dev.sh logs`.
...
It also assumes distributor IDs from the script payload exist in the selected tenant (`7c4e6f2e-91f4-4f9b-8f6f-3c7ab93ea001`, `7c4e6f2e-91f4-4f9b-8f6f-3c7ab93ea002`), or that you update the IDs in `seed.sh` before running it.
